...
import { scorePassword, strengthLevel, verhoeffValidate, validPhone } from "../utils/validators"
...
async function save() {
  try {
    if (!verhoeffValidate(aadhaar)) {
      alert("Invalid Aadhaar (simulated local verification).")
      return
    }
    ...
    // ✅ Also save locally to AppStore so ArtisanDashboard can read it
    AppStore.setProfile("artisan", profile)
    AppStore.setUser({ ...user, role: "artisan" })
            ...
            <>
              <div className="row mt-3">
                <div>
                  ...
                  />
                  <div className="helper mt-2">
                    Local validation using Verhoeff algorithm (demo). For production, integrate secure KYC APIs.
                  </div>
                </div>
                <div>
